package store.esgseed.api.soccer.stadium.controller;
...
import store.esgseed.api.soccer.stadium.domain.StadiumDTO;
import store.esgseed.api.soccer.stadium.service.StadiumService;
...
public class StadiumController {
    ...
    @PostMapping
    public Messenger<StadiumDTO> create(
            @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    description = "생성할 경기장 데이터", required = true)
            @RequestBody StadiumDTO dto) {
        StadiumDTO response = stadiumService.create(dto);
    ...
    @GetMapping("/{id}")
    public Messenger<StadiumDTO> getById(
            @Parameter(description = "조회할 경기장 ID", required = true)
            @PathVariable Long id) {
        StadiumDTO response = stadiumService.getById(id);
        return Messenger.ok(response);
    }

    @Operation(summary = "경기장 데이터 목록 조회",
               description = "모든 경기장 데이터를 조회하거나 경기장명/홈팀으로 검색합니다.")
    @ApiResponse(responseCode = "200", description = "조회 성공")
    @GetMapping
    ...
    @PutMapping("/{id}")
    public Messenger<StadiumDTO> update(
            @Parameter(description = "수정할 경기장 ID", required = true)
            @PathVariable Long id,
            @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    description = "수정할 경기장 데이터", required = true)
            @RequestBody StadiumDTO dto) {
        StadiumDTO response = stadiumService.update(id, dto);
    ...
    public Messenger<Void> delete(
            @Parameter(description = "삭제할 경기장 ID", required = true)
            @PathVariable Long id) {
